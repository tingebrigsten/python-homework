...
* The slides for this lesson can be viewed on Google Drive here: [Lesson Slides](https://docs.google.com/presentation/d/1dLAG4cycZLIDJz1q5g15NfKl_AmdXpOktece2JMnumE/edit?usp=sharing).
...
* Fetching the years and the Presidents' last names from the document IDs may be tricky, but this task is easy to face using list comprehensions and the `split()` string's method.
...
* Now that we have all the data about the most used adjectives in the inaugural addresses over time, we create a DataFrame and plot a line chart to analyze how U.S. presidents used them in their speech visually.
...
In this activity, students will create a sentiment index from News API headlines and correlate it to S&P 500 daily returns, looking for a text topic that generates the highest correlation.
  ...
  **Note:** Point out to students to be mindful of limitations imposed by the API provider regarding the number of requests they can make. For instance, the 3 requests provided in the solution will only run once within 24 hours.
